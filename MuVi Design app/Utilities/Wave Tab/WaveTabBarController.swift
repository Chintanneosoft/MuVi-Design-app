...
open class WaveTabBarController: UITabBarController, WaveTabBarProtocol {
    ...
    private struct Constants {
        ...
        static let whiteBackgroundColor = UIColor(red: 1.0, green: 1.0, blue: 1.0, alpha: 1)
    ...
    func setupCurve(_ radius: Float) {
        let path = UIBezierPath.createCurve(at: tabBarItems[safeSelectedIndex].center.x, radius: CGFloat(radius+5), on: tabBar)
    ...
    func setupCircle(_ width: Float) {
        ...
        circle?.center = CGPoint(x: tabBarItems[safeSelectedIndex].center.x, y: 10)
        circle?.layer.borderWidth = Constants.borderWidth
        circle?.layer.borderColor = Constants.borderColor
        circle?.applyGradient(colours: [UIColor(red: 239.0/255.0, green: 48.0/255.0, blue: 151.0/255.0, alpha: 1),UIColor(red: 61.0/255.0, green: 68.0/255.0, blue: 234.0/255.0, alpha: 1)],angle: 90)
    ...
    func moveCurve(with duration: TimeInterval, to index: Int, with radius: Float) {
        ...
        let endPath = UIBezierPath.createCurve(at: tabBar.subviews[safeIndex].center.x, radius: CGFloat(radius + 5), on: tabBar)
        CATransaction.begin()
